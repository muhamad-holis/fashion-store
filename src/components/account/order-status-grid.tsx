...
const items = [
  ...
  { key: "cancelled", label: "Return & Refund", icon: RotateCcw, tab: "cancelled" },
] as const;

export function OrderStatusGrid({ counts }: { counts: OrderStatusCounts }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="grid grid-cols-6 gap-1.5">
        {items.map(({ key, label, icon: Icon, tab }, i) => {
          const count = counts[key as keyof OrderStatusCounts];
          return (
            <Link
              key={key}
              href={`/akun/pesanan?status=${tab}`}
