-- =========================================================
-- 0017: Biaya Layanan (service fee) - persentase dari subtotal
-- =========================================================
-- Fitur baru: biaya layanan yang dihitung sebagai persentase dari
-- subtotal, berlaku untuk SEMUA metode pembayaran (termasuk COD).
--
-- 1. settings.service_fee_percent - persen biaya layanan yang diatur
--    admin (mis. 2 = 2%). Default 0 supaya toko yang sudah live tidak
--    tiba-tiba mengenakan biaya tanpa admin mengaktifkannya secara sadar.
-- 2. orders.service_fee - nominal Rupiah biaya layanan yang FINAL,
--    dihitung & dibekukan di server saat order dibuat (bukan dihitung
--    ulang dari persen yang mungkin berubah setelahnya), sama seperti
--    pola subtotal/discount_total/shipping_cost yang sudah ada.
-- 3. create_order_atomic() diperbarui: subtotal dihitung seperti biasa,
--    lalu v_service_fee = round(subtotal * service_fee_percent / 100),
--    dan ditambahkan ke grand_total SEBELUM validasi batas COD - supaya
--    batas COD tetap mengacu ke total yang benar-benar harus dibayar.
-- =========================================================
...
declare
  ...
  v_service_fee_percent numeric := 0;
...
begin
  ...
  -- Biaya Layanan: persentase dari subtotal (SEBELUM diskon voucher),
  -- berlaku untuk semua metode pembayaran termasuk COD. Persennya
  -- diambil dari settings dan nominal finalnya dibekukan di kolom
  -- orders.service_fee supaya perubahan persen di kemudian hari tidak
  -- mengubah nominal order yang sudah dibuat.
  select coalesce(service_fee_percent, 0) into v_service_fee_percent from settings where id = 1;
  v_service_fee := round(v_subtotal * v_service_fee_percent / 100, 0);

  v_grand_total := greatest(0, v_subtotal - v_discount + v_service_fee + coalesce(p_shipping_cost, 0));

  -- COD: order langsung 'processing' (skip 'unpaid' - tidak ada bukti
  -- transfer yang perlu diverifikasi), dan grand_total (yang sudah
  -- termasuk biaya layanan) divalidasi ulang terhadap batas maksimal COD
  -- yang diatur admin (proteksi risiko, authoritative di server - lihat
  -- catatan di kepala file migration 0016).
  ...
  insert into orders (
    ...
    subtotal, discount_total, service_fee, total_weight_grams, grand_total,
    ...
    v_subtotal, v_discount, v_service_fee, v_total_weight, v_grand_total,
